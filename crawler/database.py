...
Supports transactional writes, foreign keys, child record deduplication,
...
class CrawlDatabase:
    ...
    def _get_connection(self):
        """Yield an active SQLite connection with foreign keys enabled."""
    ...
    def save_session(
        ...
        """
        Atomically save a complete crawl session and all associated records.
        If a session_id already exists, previous child records are cleanly cleared
        before inserting new ones to prevent duplicate rows.
        """
        try:
            with self._get_connection() as conn:
                with conn:
                    ...
                    # Prevent duplicate child rows on re-saving
                    cursor.execute("DELETE FROM failed_urls WHERE session_id = ?", (summary.session_id,))
                    cursor.execute("DELETE FROM crawled_pages WHERE session_id = ?", (summary.session_id,))
            ...
            logger.error("Failed to save crawl session %s: %s", summary.session_id, e)
            return False
    ...
    def get_all_sessions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to retrieve sessions list: %s", e)
            return []
    ...
    def get_session_pages(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to retrieve pages for session %s: %s", session_id, e)
            return []
    ...
    def get_session_failures(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to retrieve failures for session %s: %s", session_id, e)
            return []

    def delete_session(self, session_id: str) -> bool:
        """Delete a crawl session and all cascaded page and failure records."""
        try:
            with self._get_connection() as conn:
                with conn:
                    cursor = conn.cursor()
