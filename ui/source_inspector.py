...
def render_source_inspector(search_result: Optional[SearchPipelineResult] = None):
    ...
    if not search_result or not search_result.extracted_evidence:
        st.markdown("""
            <div class="standby-panel-glass" style="margin-top: 1rem;">
                <div style="font-size: 0.72rem; font-weight: 700; color: #A855F7; letter-spacing: 0.09em; text-transform: uppercase; margin-bottom: 0.3rem;">
                ...
                <p style="color: #94A3B8; font-size: 0.85rem; line-height: 1.5; margin-bottom: 0;">
                    Execute a web search query in the <b>🔎 Search &amp; Research</b> workspace to inspect the full text passages,
    ...
    st.markdown("""
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 0.5rem; margin-bottom: 0.8rem; flex-wrap: wrap; gap: 0.5rem;">
            <div>
                ...
                <h3 style="margin: 0; color: #FFFFFF; font-weight: 800; font-size: 1.3rem;">Deep Source &amp; Passage Evidence Inspector</h3>
            </div>
            <div style="font-size: 0.75rem; color: #94A3B8;">
                Inspect extracted text blocks, heading contexts, and ground-truth relevance
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    status_color = "#10B981" if target_source.fetch_success else "#F59E0B"
    ...
    st.markdown(f"""
        <div class="kpi-card" style="padding: 1.1rem 1.3rem; margin-bottom: 1.2rem; border-top: 3px solid #A855F7;">
            <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 0.8rem; margin-bottom: 0.8rem;">
                <div>
                    <div style="font-size: 0.70rem; font-weight: 700; color: #A855F7; letter-spacing: 0.08em; text-transform: uppercase; margin-bottom: 0.2rem;">
                    ...
                    <div style="font-size: 0.78rem; font-family: 'JetBrains Mono', monospace; word-break: break-all;">
                        <a href="{safe_href}" target="_blank" rel="noopener noreferrer" style="color: #FB923C; text-decoration: underline;">
                            {safe_url_text} ↗
                        </a>
                    </div>
                </div>
                <div style="text-align: right;">
                    <span style="background: rgba(168, 85, 247, 0.12); border: 1px solid #A855F7; color: #C084FC; padding: 0.25rem 0.6rem; border-radius: 6px; font-size: 0.72rem; font-weight: 700;">
                        {safe_type}
                    </span>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 0.8rem; border-top: 1px solid rgba(255, 255, 255, 0.08); padding-top: 0.8rem;">
                ...
                <div>
                    ...
                    <div style="color: #C084FC; font-weight: 700; font-size: 0.82rem;">{len(target_source.passages)} blocks</div>
                </div>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    for p_idx, p in enumerate(passages, start=1):
        heading_badge = f"<span style='background: rgba(168, 85, 247, 0.15); border: 1px solid #8B5CF6; color: #D8B4FE; padding: 0.15rem 0.5rem; border-radius: 4px; font-size: 0.68rem; font-weight: 700;'>§ {html.escape(p.heading_context)}</span>" if p.heading_context else ""
        rel_color = "#10B981" if p.relevance_score >= 0.6 else ("#F59E0B" if p.relevance_score >= 0.3 else "#94A3B8")
        safe_text = html.escape(p.text)

        st.markdown(f"""
            <div style="background: rgba(16, 20, 34, 0.75); border: 1px solid rgba(255, 255, 255, 0.08); border-left: 3px solid #A855F7; border-radius: 8px; padding: 0.85rem 1.1rem; margin-bottom: 0.65rem;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.45rem; flex-wrap: wrap; gap: 0.4rem;">
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        <span style="color: #A855F7; font-family: 'JetBrains Mono', monospace; font-size: 0.72rem; font-weight: 700;">#{p_idx}</span>
                        {heading_badge}
                    </div>
